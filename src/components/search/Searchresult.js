...
<div className="content-container">
    ...
    <div className="category-content">
        {/* User content */}
        <h4>{}</h4>
{<div className='community-users'>{result?.users?.map((people,index)=>{return <UserCard  username={people.username} profile={people.profilePicture} userId={people._id} key={index}/>})}</div>}
...
<div className="category-content">
{/* Community content */}
<h4>{result.community.length!==0?result.community.length+" result found in community":"No result found"}</h4>
{<div className='right-sidebar'>{result?.community?.map((index)=>{return <TrendingTags img={index.image} name={index.name} peoples={index.users} key={index.name}></TrendingTags>})}</div>}
