...
export const UserCard = ({profile,username,userId,bio,worksAt}) => {
  ...
  return (
    ...
    <img src={process.env.REACT_APP_PUBLIC_FOLDER+profile} alt="ravindra"></img>
    <div className='user-card-info'>
        <Link to={`/profile/${userId}`}><h5>{username}</h5></Link>
        
    </div>
    <p>{bio}</p>
    <strong>{worksAt}</strong>
    {follow  ? <button  onClick={handleclickFollow} > {"Follow "}<BsFillPersonPlusFill /></button>:<button  onClick={handleclickUnFollow} > {"Unfollow "}<BsFillPersonDashFill /></button>}
